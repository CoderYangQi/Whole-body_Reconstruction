...
def main():
    ...
    exp_dir = r"Z:\users\yq\MorphDatasets\model\TransMorph\voxelmorph\0217_sim\experiments"
    log_dir = r"Z:\users\yq\MorphDatasets\model\TransMorph\voxelmorph\0217_sim\logs"
    ...
    val_loader = DataLoader(val_set, batch_size=valid_batch_size, shuffle=False, num_workers=4, pin_memory=True, drop_last=True)
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            for n, loss_function in enumerate(criterions):
                if ct == 2: # calculate the dice loss
                    curr_loss = loss_function(def_out.long(), y_seg.long(), labelNums) * weights[n]
                    print(f"dice loss : {curr_loss}")
            ...
            for n, loss_function in enumerate(criterions):
                if ct == 2:  # calculate the dice loss
                    curr_loss = loss_function(def_out.long(), x_seg.cuda().long(), labelNums) * weights[n]
                    print(f"dice loss : {curr_loss}")
