...
def TestNulti():
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            def_seg = reg_model([x_seg.cuda().float(), flows[-1].cuda()])
            ...
            loss_dsc = criterion_dsc(def_seg.cuda().long(), y_seg.cuda().long(), num_class) * weights[1]
            print(f"dsc time is {time.time() - start_dsc}")
            ...
            # def_segs = []
            # for i in range(36):
            #     def_seg = model.reconstruction(y_seg_oh[:, i:i + 1, ...].float(), flows[-1].float())
            #     def_segs.append(def_seg)
            # def_seg = torch.cat(def_segs, dim=1)

            def_seg = reg_model([y_seg.cuda().float(), flows[-1].cuda()])

            loss_ncc = criterion_ncc(warped[-1], x) * weights[0]
            loss_dsc = criterion_dsc(def_seg.gpu().long(), x_seg.gpu().long(), num_class) * weights[1]
            loss_reg = sum([criterion_reg(flow, flow) for flow in flows]) * weights[2] / 2
            loss = loss_ncc + loss_dsc + loss_reg
            loss_all.update(loss.item(), x.numel())
            # compute gradient and do SGD step
            optimizer.zero_grad()
            del def_seg, x, y, warped, flows
            gc.collect()
