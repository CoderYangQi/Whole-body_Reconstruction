...
class VISoRBrain:
    ...
    def load(self, file_path):
        ...
        with open(file_path, 'r') as info_file:
            _info = yaml.safe_load(info_file)
    ...
    def get_brain_position_from_slice(self, slice_index, slice_pos, accuracy=0.001, max_iteration=100):
        pos = [(self.slice_spheres[slice_index][0][i] + self.slice_spheres[slice_index][1][i]) / 2 for i in range(3)]
        for i in range(max_iteration):
            ...
            pos = [pos[j] - D[j] * d[j] / 0.05 for j in range(3)]
...
if __name__ == '__main__':
    brain = VISoRBrain()
    brain.load('D:/chaoyu/Test/Thy1-6521/Thy1-6521')
    import cProfile
    import tifffile
    def prof():
        brain.create_sphere_map()
        brain.save('D:/chaoyu/Test/Thy1-6521/Thy1-6521')
        #sphere_map = brain.create_sphere_map()
        #tifffile.imsave('D:/chaoyu/Test/Thy1-6521/sphere.tif', sphere_map)
    #prof()
    cProfile.run('prof()')
    #brain.save('D:/chaoyu/Test/CTB-brain/CTB_SC_488nm_.tar')
