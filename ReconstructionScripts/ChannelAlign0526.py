...
def CalChannelTranslate(prev_surface, next_surface):
    ...
    tp_ = translate_get_align_transform(next_surface, prev_surface,
                                        [os.path.join(PARAMETER_DIR,
                                                      'yq_channel_align_surface_2D.txt')])
...
def Task(refImgPath,imgPath,saveImgPath,saveRoot,sliceIndex):
    ...
    maxRefImg = sitk.MaximumProjection(refImg[:, :, 75:175], projectionDimension=2)[:, :, 0]
    maxImg = sitk.MaximumProjection(img[:, :, 75:175], projectionDimension=2)[:, :, 0]
    tp_ = CalChannelTranslate(maxRefImg, maxImg)
    # offsetDict[sliceIndex] = tp_

    img = ApplyTranslate(img, tp_)
    sitk.WriteImage(img[:, :, 100], os.path.join(saveRoot, "refineImg_{:03d}.tif".format(sliceIndex)))
    sitk.WriteImage(refImg[:, :, 100], os.path.join(saveRoot, "refImg_{:03d}.tif".format(sliceIndex)))
...
def Step1():
    ...
    refImgFormat = os.path.join(root,"2_1_1_{:03d}_561nm_10X.tif")
    ImgFormat = os.path.join(root,"2_1_1_{:03d}_648nm_10X.tif")
    saveRoot = r"D:\USERS\yq\TH2_Reconstruction\Reconstruction\TestTemp\RefineSliceImage"
    taskChunk = []
    for sliceIndex in range(77,78):
        saveImgPath = os.path.join(saveRoot, "2_1_1_{:03d}_648nm_10X.tif".format(sliceIndex))
