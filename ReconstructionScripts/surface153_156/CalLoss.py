...
from common_script.Torch_Losses import NCC,NCC_CPU,ssim3D
import torch, os
...
def ReadOffsetTxt(txtPath = r"Z:\users\yq\MorphDatasets\TestTemp\th2_33\tf_33_pars.txt"):
    ...
    for key, value in offsets.items():
        ...
        j = key[1];
    return offsets
def CalLossGPU(Offsets,FixedPathFormat, MovedPathFormat, FixedsaveRefineFormat,MovingsaveRefineFormat,rate = 1):
    ...
    for key, value in Offsets.items():
        ...
        ncc_loss = NCC()  # 确保NCC模块在CUDA上运行
        ssim_loss = ssim3D(tensor_img1, tensor_img2)
        ssim_loss_self = ssim3D(tensor_img1, tensor_img1)
        ssimList[key] = ssim_loss

        # 计算两个图像之间的NCC误差
        loss = ncc_loss(tensor_img1, tensor_img2)
        lossList[key] = loss
        # selfloss = ncc_loss(tensor_img1, tensor_img1)
        print(f"key {key} is NCC loss:{loss.item()}; ssim loss is {ssim_loss}; value is {value}")
...
class Calloss(unittest.TestCase):
    ...
    def test_loss(self):
        ...
        root = r"Z:\Data\E\E-123\Reconstruction\saveTemp\th0630_153_155"
        ...
        for slice_index in range(153, 156):
            ...
            # lossList,ssimList = CalNCC(Offsets,fixedFormat, movedFormat, FixedsaveRefineFormat,MovingsaveRefineFormat)
            lossList, ssimList = CalLossGPU(Offsets, fixedFormat, movedFormat, FixedsaveRefineFormat,
            ...
            for key, value in Offsets.items():
                ...
                i = key[0];
                j = key[1]
                # if lossList[key] < -0.40:
                if ssimList[key] > 0.40:
                    ct += 1
                    print(f"ct is {ct} key is {key}; value is {value}; loss is {lossList[key]} ssim is {ssimList[key]}")
