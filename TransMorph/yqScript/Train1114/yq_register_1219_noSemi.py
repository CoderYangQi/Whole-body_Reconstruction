...
def main():
    ...
    save_dir = r'Z:\users\yq\MorphDatasets\Bspine\1114Data\TestResult'
...
# todo load test files
    ...
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True,
                              drop_last=True)
    loss_dict = {}
    save_tif = r"Z:\users\yq\MorphDatasets\Bspine\0305_visor_pic\trans"
    ...
    with torch.no_grad():
        ...
        for data in train_loader:
            ...
            # np.savez(os.path.join(save_tif, ('disp_{:04d}' + '.npz').format(stdy_idx)), flow)
    ...
    # 保存为 JSON 文件
    with open("trans_simple_loss.json", "w") as file:
