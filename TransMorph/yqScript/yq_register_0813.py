...
def main():
    ...
    save_dir = r'Z:\users\yq\MorphDatasets\model\TransMorph\0813\test_result'
...
# todo load test files
    ...
    train_num = 0
    val_num = 16
    fixed_root = r"Z:\users\yq\MorphDatasets\Bspine\0811\fixed"
    moving_root = r"Z:\users\yq\MorphDatasets\Bspine\0811\moving"
    ext = '.tif'
    train_fixed_list = get_files(fixed_root, ext)[:train_num]
    train_moving_list = get_files(moving_root, ext)[:train_num]
    ...
    labelEXT = '.nii'
    valid_fixed_label_files = get_files(fixed_label_root, labelEXT)[train_num:val_num + train_num]
    # fixed_label_files = sorted(fixed_label_files)
    valid_moving_label_files = get_files(moving_label_root, labelEXT)[train_num:val_num + train_num]
    ...
    with torch.no_grad():
        ...
        for data in val_loader:
            ...
            #                 os.path.join(save_dir,('x_def_{:04d}'+ext).format(stdy_idx)))
            # sitk.WriteImage(sitk.GetImageFromArray(x),
            #                 os.path.join(save_dir,('x_{:04d}'+ext).format(stdy_idx)))
            # sitk.WriteImage(sitk.GetImageFromArray(y),
            #                 os.path.join(save_dir,('y_{:04d}'+ext).format(stdy_idx)))
