...
def plot_comparison_single_model(model_name, semi_loss_data, no_semi_loss_data):
    # 定义损失名称
    loss_names = ["NCC", "SSIM", "MI", "MSE", "Dice"]
    ...
    for i in range(num_losses):
        ...
        plt.bar(['半监督', '无监督'], [semi_value, no_semi_value], color=['skyblue', 'salmon'])
...
class TestLossND(unittest.TestCase):
    ...
    def test_plot_comparison(self):
        ...
        semi_file_paths  = {
            ...
            'vxm': r"D:\USERS\yq\code\TransMorph_Transformer\VoxelMorph_YQ\vxm_semi_0210_loss.json"
        }
    ...
    def test_plot_LABEL_comparison(self):
        # 设置中文字体
        plt.rcParams['font.sans-serif'] = ['Microsoft YaHei']  # 或者 'SimHei'
        plt.rcParams['axes.unicode_minus'] = False  # 解决负号显示问题
        ...
        # 定义文件路径
        semi_file_paths  = {
            'trans': r"D:\USERS\yq\code\TransMorph_Transformer\TransMorph\yqScript\Train_0210_Semi\trans_0217_semi_loss_dice.json",
        }
    ...
    def test_get_losses(self):
        trans_paths  = {
            'simple': r"D:\USERS\yq\code\TransMorph_Transformer\TransMorph\yqScript\Train_noSemi_0211\trans_0217_noSemi_dice_loss.json",
            'dice': r"D:\USERS\yq\code\TransMorph_Transformer\TransMorph\yqScript\Train_0210_Semi\trans_0217_semi_loss_dice.json",
        }
        for key,value in trans_paths.items():
            res = process_loss_avg(value)
            print(f"type {key}: NCC {res[0]}; SSIM {res[1]}; MI {res[2]}; MSE {res[3]}; Dice {res[4]}")