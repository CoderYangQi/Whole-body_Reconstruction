...
class RecursiveCascadeNetwork(nn.Module):
    def __init__(self, n_cascades, config):
        ...
        # self.stems.append(TransMorph(config))
        for i in range(n_cascades):
            self.stems.append(TransMorph(config))
        self.reconstruction = SpatialTransformer(config.img_size)
        ...
        # self.reconstruction.cuda()

    def forward(self, moving, fixed):
        ## in the transfmorph, x is fixed image; y is moving image
        flows = []
        stem_results = []
        stem_results.append(moving)
        # # Affine registration
        # flow = self.stems[0](fixed, moving)
        # stem_results.append(self.reconstruction(moving, flow))
        # flows.append(flow)
        for model in self.stems[:]: # cascades
            # registration between the fixed and the warped from last cascade
            x_in = torch.cat((stem_results[-1], fixed), dim=1)
            out, flow = model(x_in)
            stem_results.append(out)
            flows.append(flow)

        return stem_results, flows


    def load_all_state(self, best_model):
        ct = 0
        name_format = 'cascade {}'
        for model in self.stems:
            # model.to(device)
            model.load_state_dict(best_model[name_format.format(ct)])
            ct+=1


# 512 * 512 * 64 分开多尺度
class RecursiveCascadeNetwork_512(nn.Module):
    def __init__(self, n_cascades, config):
        super(RecursiveCascadeNetwork_512, self).__init__()
        ...
        for i in range(n_cascades):
            self.stems.append(TransMorph(config))
        self.reconstruction = SpatialTransformer((64, 512, 512))
    ...
    def forward(self, moving, fixed):
        ...
        # moving_0 = moving[:,:,:,::2,::2]
        # fixed_0 = fixed[:,:,:,::2,::2]
        # x_in_0 = torch.cat((moving[:,:,:,::2,::2], fixed[:,:,:,::2,::2]), dim=1)
        out, flow = self.stems[0](torch.cat((moving[:,:,:,::2,::2], fixed[:,:,:,::2,::2]), dim=1))
        up_flow = F.interpolate(flow, scale_factor=(1, 2, 2), mode='trilinear', align_corners=False)
        # up_out = F.interpolate(out, scale_factor=(1, 2, 2), mode='trilinear', align_corners=False)
        up_out = self.reconstruction(moving, up_flow)
        stem_results.append(up_out)
        flows.append(up_flow)
        del out, flow
        ...
        # sub 2
        # 分割张量
        # top_left = tensor[:, :, :, :256, :256]  # 左上角
        # top_right = tensor[:, :, :, :256, 256:]  # 右上角
        # bottom_left = tensor[:, :, :, 256:, :256]  # 左下角
        # bottom_right = tensor[:, :, :, 256:, 256:]  # 右下角

        # 初始化输出张量，与输入形状一致
        # flow = torch.zeros_like(up_flow)
        # out = torch.zeros_like(up_out)

        # 用于暂存结果的 numpy 数组
        flow_numpy = np.zeros((1, 3, 64, 512, 512), dtype=np.float32)
        out_numpy = np.zeros((1, 1, 64, 512, 512), dtype=np.float32)


        flow_list = []
        out_list = []
        regions = []
        for i in range(2):  # 高度方向分为 2 段 (0-256, 256-512)
            for j in range(2):  # 宽度方向分为 2 段 (0-256, 256-512)
                region = (slice(None), slice(None), slice(None), slice(i * 256, (i + 1) * 256), slice(j * 256, (j + 1) * 256))
                regions.append(region)
                sub_input = torch.cat((up_out[region]
                                       , fixed[region]), dim=1)

                # 模型前向计算
                sub_out, sub_flow = self.stems[1](sub_input)

                # 将结果从 GPU 张量转换为 numpy
                # sub_out_numpy = sub_out.cpu().detach().numpy()
                sub_flow_numpy = sub_flow.cpu().detach().numpy()

                # 存入 numpy 暂存数组
                flow_numpy[region] = sub_flow_numpy
                # out_numpy[region] = sub_out_numpy

                # 清理显存
                del sub_input, sub_out, sub_flow
                torch.cuda.empty_cache()

                # flow_list.append(sub_flow)
                # out_list.append(sub_out)
        # # # 手动清理显存
        # del sub_input, sub_flow, sub_out
        torch.cuda.empty_cache()
        gc.collect()
        flow = torch.from_numpy(flow_numpy).cuda()
        out = self.reconstruction(up_out, flow)
...
class CascadeTransMorph(nn.Module):
    def __init__(self, trans_morph_model, num_stages=2):
        super(CascadeTransMorph, self).__init__()
        self.num_stages = num_stages
        self.models = nn.ModuleList([trans_morph_model() for _ in range(num_stages)])

    def forward(self, moving_image, fixed_image):
        deform_fields = []  # 存储每个阶段的变形场
        deformed_images = []  # 存储每个阶段的变形图像

        x = moving_image
        for stage in range(self.num_stages):
            model = self.models[stage]
            # 假设 TransMorph 返回 (deformed_image, deform_field)
            deformed_image, deform_field = model(x, fixed_image)
            deform_fields.append(deform_field)
            deformed_images.append(deformed_image)
            # 将当前阶段的输出作为下一阶段的输入
            x = deformed_image

        return deformed_images, deform_fields
CONFIGS = {
