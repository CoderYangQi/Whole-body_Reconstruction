...
def combine_transforms_xy_z(tf_xy, tf_z):
    ...
    print(f"tf_z {tf_z.GetNumberOfComponentsPerPixel()}")
    print(f"tf_xy {tf_xy.GetNumberOfComponentsPerPixel()}")
    if tf_z.GetNumberOfComponentsPerPixel() != tf_xy.GetNumberOfComponentsPerPixel():
...
def yq_create_brain_(input_, internal_pixel_size, slice_thickness,slice_offset_list ,output_path=None):
    ...
    for i in ud:
        ...
        # u = sitk.Compose(sitk.VectorIndexSelectionCast(u, 0) * internal_pixel_size - slice_offset[0] + sl.sphere[0][0],
        #                  sitk.VectorIndexSelectionCast(u, 1) * internal_pixel_size - slice_offset[1] + sl.sphere[0][1],
        #                  sitk.VectorIndexSelectionCast(u, 2) * internal_pixel_size + (- (i - 1) * slice_thickness) + sl.sphere[0][2])
        # l = sitk.Compose(sitk.VectorIndexSelectionCast(l, 0) * internal_pixel_size - slice_offset[0] + sl.sphere[0][0],
        #                  sitk.VectorIndexSelectionCast(l, 1) * internal_pixel_size - slice_offset[1] + sl.sphere[0][1],
        #                  sitk.VectorIndexSelectionCast(l, 2) * internal_pixel_size + (- i * slice_thickness) + sl.sphere[0][2])
