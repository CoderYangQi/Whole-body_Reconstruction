...
# 计算初始坐标
def create_reference(rawdata: RawData, flip=False) -> VISoRSample:
...
def reconstruct_sample(
        rawdata: RawData,
        methods: typing.Union[str, dict] = 'default',
        reference_slice: VISoRSample = None,
        overwrite_pixel_size=None,
        flip=False,
        *args, **kwargs) -> VISoRSample:
    if methods == 'default':
        methods = {'stitch': 'default'}
    if methods == 'elastix_align':
        methods = {'stitch': 'elastix_align'}
    if overwrite_pixel_size is not None:
        rawdata = RawData(rawdata.file, overwrite_pixel_size=overwrite_pixel_size)
    sample_data = VISoRSample()
    sample_data.column_source = rawdata.file
    sample_data.column_images = get_column_images(rawdata)
    sample_data = create_reference(rawdata, flip=flip)
