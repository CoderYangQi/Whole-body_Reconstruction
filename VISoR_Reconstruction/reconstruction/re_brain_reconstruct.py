...
def calc_surface_height_map(img: sitk.Image, slice_thickness=300, internal_pixel_size=4, internal_downsample=(2, 2, 1),
    ...
    for i in range(8000):
        def calc_grad(s, grad, grad_m, ul):
            ...
            grad = metric_weight[0] * u_edge + \
                   (metric_weight[1] / surface_pixel_size) * u_plane_bend + \
                   (metric_weight[2] / thickness * ul) * torch.clamp(l - u - thickness, -1000, 10) + \
                   momentum * grad
            return grad, g
    ...
    umap = np.float32((u * internal_downsample[2]).cpu().numpy()[0][0])
    lmap = np.float32((l * internal_downsample[2] - 1).cpu().numpy()[0][0])
...
def combine_transforms(tf1, tf2):
    size1 = tf1.GetSize()
    size = tf2.GetSize()
    size1_ = [max(size1[i], int(size[i] * 1.2)) for i in range(3)]
    origin = [(size1[i] - size1_[i]) // 2 for i in range(3)]
    v = []
    for i in range(3):
        t = sitk.ZeroFluxNeumannPad(sitk.VectorIndexSelectionCast(tf1, i), [-origin[0], -origin[1], 0], [-origin[0], -origin[1], 0])
        v.append(t)
        t.SetOrigin(origin)
    tf1 = sitk.Compose(v)
...
def optimize_transforms(u, l, i_pyr):
    ...
    for i in uu_:
        for j in range(2):
            ...
            uu_[i][j] = uu_[i][j].cuda()
            lu_[i][j] = lu_[i][j].cuda()
    ...
    for j in uu_:
        if ct == 0:
            ct = ct + 1
            uu = sitk.Compose([sitk.GetImageFromArray(uu_[j][k].cpu().numpy()) for k in range(2)])
            lu = sitk.Compose([sitk.GetImageFromArray(lu_[j][k].cpu().numpy()) for k in range(2)])
            uu_[j] = uu
            lu_[j] = lu
            continue
        elif ct == 1:
            ct = ct + 1
            # container
            # uu_temp = [Du(uu_[j][0]), Du(uu_[j][1])]
            uu_temp = [uu_[j][0].clone(), uu_[j][1].clone()]

            # container
            uu_[j] = uu_[j]
            lu_[j] = [uu_[j][0] + lu_[j][0], uu_[j][1] + lu_[j][1]]
            # uu_temp = [uu_temp[0] + uu_[j][0], uu_temp[1] + uu_[j][1]]
            uu = sitk.Compose([sitk.GetImageFromArray(uu_[j][k].cpu().numpy()) for k in range(2)])
            lu = sitk.Compose([sitk.GetImageFromArray(lu_[j][k].cpu().numpy()) for k in range(2)])
            uu_[j] = uu
            lu_[j] = lu
            ...
            uu_[j] = [uu_[j][0] + uu_temp[0].clone(),uu_[j][1] + uu_temp[1].clone()]
            uu_temp = [uu_[j][0].clone(), uu_[j][1].clone()]
            # lu_temp = [uu_temp[0] + uu_[j][0],uu_temp[1] + uu_[j][1]]
            # uu_[j] = uu_temp
            lu_[j] = [uu_[j][0].clone() + lu_[j][0], uu_[j][1].clone() + lu_[j][1]]
            # uu_temp = [uu_temp[0] + uu_[j][0], uu_temp[1] + uu_[j][1]]
            uu = sitk.Compose([sitk.GetImageFromArray(uu_[j][k].cpu().numpy()) for k in range(2)])
            lu = sitk.Compose([sitk.GetImageFromArray(lu_[j][k].cpu().numpy()) for k in range(2)])
            uu_[j] = uu
...
def create_brain_(input_, internal_pixel_size, slice_thickness, output_path=None):
    ...
    for i in ud:
        ...
        u = sitk.Compose(sitk.VectorIndexSelectionCast(u, 0) * internal_pixel_size + sl.sphere[0][0],
                         ...
                         sitk.VectorIndexSelectionCast(u, 2) * internal_pixel_size + (sl.sphere[0][2] - (i - 1) * slice_thickness) - 0.1)
        l = sitk.Compose(sitk.VectorIndexSelectionCast(l, 0) * internal_pixel_size + sl.sphere[0][0],
                         ...
                         sitk.VectorIndexSelectionCast(l, 2) * internal_pixel_size + (sl.sphere[0][2] - i * slice_thickness) + 0.1)
...
def generate_brain_image(brain: VISoRBrain, img, slice_index, input_pixel_size, output_pixel_size, name_format, n_start,
                         roi=None, slice_origin=None, bit_downsample=True, align_edge=True):
    if slice_origin is None:
        slice_origin = brain.slices[slice_index].sphere[0]
        print(f"slice_origin is {slice_origin}")
    ...
    size = [int((roi[1][j] - roi[0][j]) / output_pixel_size)
            for j in range(3)]
    origin = roi[0]
    spacing = [output_pixel_size for i in range(3)]
    if align_edge is True:
        origin = [origin[i] + 0.5 * output_pixel_size for i in range(3)]
        spacing = [spacing[i] * size[i] / (size[i] + 1) for i in range(3)]
    print(size)
    res = sitk.Resample(img, size, brain.transform(slice_index), sitk.sitkLinear, origin, spacing)
...
def generate_brain_projection(input_image_list, thickness, output_path):
    ...
    for i in range(0, len(input_image_list), int(thickness)):
