...
preset_param = {'mouse_fine': {'internal_pixel_size': 4, 'output_pixel_size': 2, 'slice_source': 'raw'},
                ...
                'macaque_fast': {'slice_thickness': 300, 'internal_pixel_size': 10, 'output_pixel_size': 5,
                                 'slice_template': 'general', 'surface_detect_downsample': [4, 4, 1],
                                 'global_optimization_downsample': 4},
...
def gen_brain_reconstruction_pipeline(dataset: VISoRData, preset='mouse_fast', **param):
    ...
    for b in source_d:
        ...
        for i, r in source_d[b][ref_channel].items():
            ...
            _create_task('calc_surface_height_map', slice_name,
                         {'slice_thickness': param['slice_thickness'],
                          ...
                          'internal_downsample': param['surface_detect_downsample']},
        ...
        _create_task('process_transforms', b,
                     {'internal_downsample': param['global_optimization_downsample'],
        ...
        for j in range(len(channels)):
            ...
            # Generate freesia input file
            t_fressia = _create_target('freesia_input_{}_C{}'.format(output_pixel_size, channel_name), 'file',
                                       os.path.join(rec_path, 'BrainImage', 'freesia_{}_C{}_{}.json'.format(output_pixel_size, c, channel_name)),
                                       ['BrainImage', 'FreesiaFile'])
            _create_task('generate_freesia_input', channel_name,
                         {'input_path': os.path.join(rec_path, 'BrainImage', str(output_pixel_size)),
                          'shape': [
                              int(int(roi_width / internal_pixel_size) * internal_pixel_size / output_pixel_size),
                              int(int(roi_height / output_pixel_size) * internal_pixel_size / output_pixel_size)],
                          'channel': c,
                          'pixel_size': output_pixel_size,
                          'group_size': int(param['slice_thickness'] / output_pixel_size)},
                         {k: v for k, v in all_targets.items() if k[:2] == 'd_'}, [t_fressia])
