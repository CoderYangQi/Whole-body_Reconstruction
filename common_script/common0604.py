...
def Preprocess(surface, threshold):
    ...
    surface = sitk.Clamp((sitk.Log(sitk.Cast(surface + 1, sitk.sitkFloat32)) - back_log_value) * 100,
...
def normalize(data):
    ...
    # min_val = data.min()
    # max_val = data.max()
    data_clipped = np.clip(data, 0, 40)
    data_stretched = (data_clipped / 40) * 255

    # 数据缩放到0-255并转换为uint8
    # data_scaled = np.clip(data_normalized * 255, 0, 255)
    data_scaled = sitk.GetImageFromArray(data_stretched)
    return data_scaled
...
def CalROIBlock(prev_surface, next_surface,spacing, ref_img: sitk.Image = None, prev_points=None, next_points=None,
                   outside_brightness=2, nonrigid=True, ref_size=None, ref_scale=1, use_rigidity_mask=False, **kwargs):
    size = prev_surface.GetSize()
    # temp = sitk.GetArrayFromImage(prev_surface)
    # if len(size) == 2:
    ...
    '''
    # justify the spcaing of imgs
    prev_surface.SetOrigin([0,0,0])
    next_surface.SetOrigin([0,0,0])
    prev_surface.SetSpacing(spacing)
    next_surface.SetSpacing(spacing)

    next_result, transform2 = get_align_transform(prev_surface, next_surface,
                                                  [os.path.join(PARAMETER_DIR, 'p_3D_0529.txt')])
    # next_result, transform2 = get_align_transform(prev_surface, next_surface,
    # [os.path.join(PARAMETER_DIR, 'p_affine.txt')],
    # rigidity_mask=rigidity_mask)
    # next_df = sitk.TransformToDisplacementField(transform2,
...
def SeparateBlock(img,up_img,down_img,spacing,bottom1,end2,roi, slices_index,saveRoot,
                  block_size = 250,sub_block = 250, tempName = 'th0630_153_155'):
    if img == None:
    ...
    for i in range(row):
        for j in range(col):
            ...
            if forbid_points[i, j]>0.4:
                ...
                bool_temp = max_sub_down > back_brightness
                int_temp = np.array(bool_temp, dtype=np.int)

                hollow_scale = (np.mean(int_temp))
                if hollow_scale < 0.4:
                    continue
                sub_up = Preprocess(sub_up, 120)
                sub_down = Preprocess(sub_down, 120)
                sub_up = fill(sub_up)
                sub_down = fill(sub_down)
                # sub_up = normalize(sub_up)
                # sub_down = normalize(sub_down)
                ...
                # create file folder
                create_folder(os.path.join(saveRoot,tempName))
                ...
                block_save_path = os.path.join(saveRoot, tempName, str(slices_index) + '_' + str(slices_index+1))
                if not os.path.exists(block_save_path):
                    os.mkdir(block_save_path)
                # sub_up = sub_up[::2,::2,:]
                # sub_down = sub_down[::2,::2,:]
                ...
                try:
                    prev_surface, next_result, transform2 = CalROIBlock(sub_up,sub_down,spacing)
                    print(transform2)
                    ...
                    with open(os.path.join(saveRoot, tempName,'tf_'+str(slices_index)+'_pars.txt'), 'w') as file:
                        # 将列表的每个元素写入文件的一行
    ...
    np.save(os.path.join(saveRoot, tempName,'tf_'+str(slices_index)+'_pars.npy'),vector_points)
    print(tf_pars)
    pass


def fill(img, outside_brightness=2):
    size = img.GetSize()
    temp = sitk.GetArrayFromImage(img)
    if len(size) == 2:
        temp[:, :] = fill_outside_yq(temp[:, :], outside_brightness)
    else:
        for i in range(size[2]):
            temp[i, :, :] = fill_outside_yq(temp[i, :, :], outside_brightness)
    img = sitk.GetImageFromArray(temp)
    return img
