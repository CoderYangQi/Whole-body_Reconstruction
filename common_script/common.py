...
def SeparateBlock(img,up_img,down_img,spacing,bottom1,end2,interval, slices_index,block_size = 250,sub_block = 125, tempName = 'th2_33'):
    if img == None:
    ...
    for i in range(row):
        for j in range(col):
            ...
            if forbid_points[i, j]>0.4:
                ...
                sitk.WriteImage(sub_up, os.path.join(r"D:\USERS\yq\code\cal_overlap\Refine", tempName, str(i)+"_"+str(j)+"up_temp_all.tif"))
                sitk.WriteImage(sub_down, os.path.join(r"D:\USERS\yq\code\cal_overlap\Refine", tempName, str(i)+"_"+str(j)+"down_temp_all.tif"))
